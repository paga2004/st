...
extern Glyph const styleSearch, styleCmd;
...
int executeMotion(char const cs, int len, KeySym const *const ks) {
	...
	if      (ks && *ks == XK_d) historyMove(0, 0, term.row / 2);
	...
	} else if (cs == 'g') for(int i = 0, end=histOff + term.c.y; i < end; ++i) historyMove(-term.c.x, -1, 0); 
        ...
        int startx = term.c.x, starty = term.c.y;
		for (int it=0, on=0; state.m.c > 0; ++it) {
            if (off || it) {
                int before = term.c.x;
                if (!historyMove(sgn, 0, 0)) it = mit;        //< offset move
                if(term.c.x == before) {
                    term.c.x = startx;
                    term.c.y = starty;
                    break;
            ...
            if (found && off) {
                int before = term.c.x;
                historyMove(-sgn, 0, 0);       //< offset move if required
                if(term.c.x == before) {
                    term.c.x = startx;
                    term.c.y = starty;
                    break;
...
ExitState kpressHist(char const *cs, int len, int ctrl, KeySym const *ksym) {
    ...
    int lines =  insertOff-histOff+term.row - term.c.y;
...
void historyOverlay(int x, int y, Glyph* g) {
	...
	if(overlay && term.col > 9 && term.row > 4 && (x > (2*term.col/3)) && (y >= (term.row-2))) {
		*g = (y == term.row - 2 )  ? styleSearch : 	styleCmd;
		...
		else if (x > term.col - 15) g->u = posBuffer[x - term.col + 15];
